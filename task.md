...
自動狩獵期間若出戰角色死亡或死透，該帳號 runner 會暫停狩獵並嘗試回程及移動至大草原第 1 層。它會按伺服器行動時間等待、只在全隊可完成時送出移動完成，再驗證 `/huntInfo` 位置。抵達後停止 runner，角色仍需手動重生或轉生。`POST /move/1` 已錄得 HTTP 200，回應顯示 4 名英雄在初始之鎮移動中；後續 `POST /move/complete` 確認抵達大草原 `1/1`。目的地不符就停止。
...
- [x] 新增死亡／重生流程：`hp: 0, perished: false` 為死亡，呼叫已確認的 `POST /heroes/reviveAll`；依回傳的 `actionCompleteTime` 排定狀態重讀，僅於最新回應 `canComplete: true` 時提供逐角色完成行動。任一死亡狀態會停止隊伍狩獵。
- [x] 死透了（`perished: true`）提供單角轉生：呼叫 `POST /heroes/{heroId}/reincarnate` 前二次確認，成功後重讀全隊；結果不明時不自動重試。
- [ ] 補錄個別轉生 POST 的 request body、完整 response 與轉生後角色欄位，確認目前暫採用的無 body 請求。
- [x] 有死亡或死透角色時提供「回程／返回起點」：經使用者確認後呼叫 `POST /move/0`，重讀 `/heroes` 與 `/huntInfo`，自動狩獵保持停止。
