...
介面以左側角色欄、右側帳號與自動狩獵控制、右側下方流程訊息組成。前景頁面每 15 秒自動重新讀取目前帳號的 `/heroes` 與 `/huntInfo`；流程訊息預設啟用，可關閉、清除或複製，供日常查看自動化步驟。另提供預設關閉的操作紀錄，勾選後才記錄 API 方法／路徑、成功失敗、耗時、流程分支與排程，可複製 JSON；資料只留在當前頁面且不包含 token、headers、request body 或 response body。Console 只保留給 API 細節排查。
