...
可在同一瀏覽器保存多個帳號 token；每個帳號各自保存目標樓層、HP/SP 閾值及休息設定。切換帳號會停止目前自動化、清除該頁面的舊角色資料與到期輪詢，再重新讀取新帳號的角色及狩獵位置。目前同一時間只執行一個帳號，避免未確認跨帳號併行的遊戲冷卻與 API 頻率限制。

已實作：多帳號 token 管理與各自設定、讀取多角色、全隊重複休息、目標樓層前行、到達後原地狩獵、停止與錯誤保護。

「無動作提醒分鐘」預設為 3 分鐘。若超過下一次預期動作時間仍未完成任何流程，Autoy 會取消請求、停止自動化並顯示提醒。

角色 `hp: 0, perished: false` 會標示為「死亡」；`perished: true` 會標示為「死透了」。任一狀態會停止自動狩獵，並提供經確認的「回程／返回起點」操作；API 回傳位置為大草原第 1 層，不能據此認定是城鎮。回程後重新讀取隊伍與狩獵狀態，且不會自動恢復狩獵。死亡角色可呼叫「全部重生」；死透角色可逐角轉生。
