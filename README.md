...
靜態 GitHub Pages 前端。使用者可在頁面新增帳號別名與 Bearer token；token 僅保存在該瀏覽器的 `localStorage`，不會寫入 GitHub。
...
API 必須允許 GitHub Pages 網域的 CORS，且 API token 必須能以 `Authorization: Bearer <token>` 使用。若伺服器未允許跨來源請求，瀏覽器會阻擋呼叫。
